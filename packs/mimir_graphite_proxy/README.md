...
```
nomad-pack run mimir_graphite_proxy --var message="Hola Mundo!"
```
...
balance the Consul service `mimir_graphite_proxy-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  mimir_graphite_proxy application
- `consul_service_name` (string) - The consul service name for the mimir_graphite_proxy
  application
