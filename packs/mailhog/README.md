...
```
nomad-pack run mailhog --var message="Hola Mundo!"
```
...
balance the Consul service `mailhog-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  mailhog application
- `consul_service_name` (string) - The consul service name for the mailhog
  application
