...
```
nomad-pack run mimir --var message="Hola Mundo!"
```
...
balance the Consul service `mimir-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  mimir application
- `consul_service_name` (string) - The consul service name for the mimir
  application
