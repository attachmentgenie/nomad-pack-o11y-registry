...
```
nomad-pack run prometheus_graphite_exporter --var message="Hola Mundo!"
```
...
balance the Consul service `prometheus_graphite_exporter-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  prometheus_graphite_exporter application
- `consul_service_name` (string) - The consul service name for the prometheus_graphite_exporter
  application
