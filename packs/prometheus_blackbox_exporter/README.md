...
```
nomad-pack run prometheus_blackbox_exporter --var message="Hola Mundo!"
```
...
balance the Consul service `prometheus_blackbox_exporter-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  prometheus_blackbox_exporter application
- `consul_service_name` (string) - The consul service name for the prometheus_blackbox_exporter
  application
