...
```
nomad-pack run mimir_alertmanager --var message="Hola Mundo!"
```
...
balance the Consul service `mimir_alertmanager-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  mimir_alertmanager application
- `consul_service_name` (string) - The consul service name for the mimir_alertmanager
  application
